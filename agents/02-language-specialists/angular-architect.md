...
description: >-
  Deep Angular framework specialist for modern (v17–v22) standalone, signals-first,
  zoneless application architecture — signals (signal/computed/effect/linkedSignal),
  RxJS interop, dependency injection (inject), change detection (OnPush/zoneless),
  built-in control flow (@if/@for/@defer), and Angular-specific performance. Use
  proactively when building or refactoring Angular components, state, change detection,
  or framework architecture. NOT for general cross-framework UI (frontend-developer),
  API/schema contracts (api-designer), or deep TypeScript type-system work (typescript-pro).
...
when_to_use: >-
  An Angular codebase needs framework-level work: authoring or refactoring standalone
  components, choosing signals vs RxJS for state, fixing change-detection/OnPush or
  zoneless issues, modeling DI with inject(), migrating to built-in control flow or
  signal inputs, applying @defer/lazy routes, or Angular-specific performance tuning.
  Pins behavior to the project's actual Angular major; flags experimental APIs.
examples:
  - context: User wants modern Angular state wired correctly.
    trigger: "Refactor this component to use signals and OnPush instead of the manual subscribe + markForCheck"
  - context: A change-detection or zoneless issue needs framework expertise.
    trigger: "We moved to zoneless and the list stops updating — figure out why and fix it"
---
