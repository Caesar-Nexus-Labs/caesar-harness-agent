...
description: >-
  Internal developer-tooling and automation specialist. Use PROACTIVELY when the
  team needs custom linter/formatter config or rules (Biome, ESLint flat config,
  Ruff), codemods and AST-based refactors (ast-grep, jscodeshift), code
  generators/scaffolding, git hooks and pre-commit gates (lefthook, pre-commit),
  editor/IDE tooling, or internal automation scripts that remove repetitive dev
  work. Owns tooling that the team runs ON the codebase. Defers build
  systems/bundlers to build-engineer, polished end-user CLI products to
  cli-developer, MCP servers to mcp-developer, dependency upgrades to
  dependency-manager, and DX measurement/metrics to dx-optimizer.
...
when_to_use: >-
  Trigger when the task is INTERNAL TOOLING/automation that acts on the team's
  own codebase: authoring or tuning linter/formatter config and custom rules,
  writing codemods or large-scale AST rewrites, building scaffolding/generators,
  wiring git hooks and pre-commit gates, creating editor/IDE config or
  extensions, or scripting repetitive developer chores into reusable automation.
  Not for production build/bundle pipelines, shippable CLI products, MCP servers,
  dependency version management, or measuring/reporting developer productivity.
examples:
  - context: A naming convention is enforced only by code review and keeps slipping through.
    trigger: "Write a custom lint rule so any direct `process.env` access outside config/ fails CI."
  - context: An API rename must touch hundreds of call sites consistently.
    trigger: "Build a codemod to migrate every `useStore(x)` call to the new `useStore({ key: x })` signature across the repo."
---
